...
class TestTeams(unittest.TestCase):
  ...
  def test_get_returns_all_teams(self):
    """teams endpoint should return all teams"""
    teams = test_get(self, base_url + endpoint)
    self.assertEqual(2, len(teams), 'Expected 2 teams to be returned')

  def test_post_returns_405_error(self):
    """Post requests are not allowed at teams endpoint"""
    test_post(self, base_url, expected_status=405)

  def test_put_returns_405_error(self):
    """Put requests are not allowed at teams endpoint"""
    test_put(self, base_url, expected_status=405)

  def test_delete_returns_405_error(self):
    """Delete requests are not allowed at teams endpoint"""
